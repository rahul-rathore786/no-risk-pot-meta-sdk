...
import PyusdFaucetInfo from "../components/UsdcFaucetInfo";
...
function BuyTickets({
  ...
  const handleApprove = async () => {
    ...
    setLoading(true);
    setMessage({ text: "Approving USDC...", type: "info" });
    ...
    try {
      ...
      setIsApproved(true);
      setMessage({ text: "USDC approved successfully!", type: "success" });
    } catch (error) {
      console.error("Error approving USDC:", error);
      setMessage({
        text: "Failed to approve USDC. Please try again.",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleBuyTickets = async () => {
    ...
    try {
      ...
      if (error.message.includes("allowance")) {
        setMessage({
          text: "Please approve USDC spending first.",
          type: "error",
        });
        setIsApproved(false);
      } else {
  ...
  // Show USDC Faucet Info if user has no USDC
        ...
        <div className="ticket-purchase-form">
          <div className="ticket-purchase-summary">
            <div className="summary-item">
              ...
              <p>{parseFloat(lotteryData.pyusdBalance).toFixed(2)} USDC</p>
            </div>
          ...
          <div className="ticket-cost">
            <h3>Total Cost</h3>
            <p className="cost-value">{totalCost} USDC</p>
            {!hasSufficientPyusd && (
              <p className="insufficient-funds">
                Insufficient USDC balance. Visit the USDC faucet to get more
                tokens.
              </p>
            )}
          </div>
